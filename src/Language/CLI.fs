...
module constants =
    ...
    ///     or no args are supplied.
    /// </summary>
    let HELP_STRING: string = $"{VERSION_STRING}
...
module functions =
    ...
    let help (): int32 =
        printf $"{constants.HELP_STRING}"
        0

    /// <summary>Displays the <c>VERSION_STRING</c> of this current instance of <b>Diorite</b>.</summary>
    let version (): int32 =
...
module parsing =
    ...
    /// <summary>
    ///     Parses the list of raw literal arguments (<c>argv</c>) into a concrete list of tokenized <c>Argument</c>
    ///     types.
    /// </summary>
    ...
    /// <summary>
    ///     Executes the specific <b>Diorite</b> functionality through the provided combination of <c>Argument</c>
    ///     list.
    /// </summary>
...
let main (argv: array<string>): int32 =
    let args: list<parsing.Argument> = parsing.collectArgs ( Array.toList argv )
    printf $"{ parsing.list2str<parsing.Argument> args }"
    parsing.executeArgs args
